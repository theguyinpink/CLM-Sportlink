...
export async function togglePostLikeFromClient(postId: string): Promise<ActionResult & { liked?: boolean; likeCount?: number }> {
  const supabase = await createClient();
  ...
  if (existing?.id) {
    ...
    revalidatePostSurfaces();
    return { ok: true, liked: false, likeCount: count ?? 0 };
  }
  ...
  revalidatePostSurfaces();
  return { ok: true, liked: true, likeCount: count ?? 0 };
}

export async function createPostCommentFromClient(input: {
  post_id: string;
  author_type: PostAuthorType;
  text_content: string;
  ...
  const authorType = input.author_type === "club" ? "club" : "player";
  const resolved = await resolveAuthor(authorType);
  const { supabase, user, error: authorError } = resolved;
  ...
  if (!postId) return { ok: false, error: "Publication introuvable." };
  if (!textContent) return { ok: false, error: "Le commentaire est vide." };
  if (textContent.length > 800) return { ok: false, error: "Le commentaire est trop long." };
...
export async function deletePostCommentFromClient(commentId: string): Promise<ActionResult> {
  const supabase = await createClient();
